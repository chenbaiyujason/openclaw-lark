...
function resolveFeishuSendContext(params: {
  ...
  const resolvedTarget = resolveFeishuSendTarget({
    ...
    replyInThread: params.threadId != null,
...
export const feishuOutbound: ChannelOutboundAdapter = {
  ...
  sendText: async ({ cfg, to, text, accountId, replyToId, threadId }) => {
    ...
    const result = await sendTextLark({ ...ctx, text });
  ...
  sendMedia: async ({ cfg, to, text, mediaUrl, mediaLocalRoots, accountId, replyToId, threadId }) => {
    ...
    if (text?.trim()) {
      await sendTextLark({ ...ctx, text });
    }

    // No mediaUrl — text-only fallback.
    if (!mediaUrl) {
      log.info('sendMedia: no mediaUrl provided, falling back to text-only');
      const result = await sendTextLark({ ...ctx, text: text ?? '' });
      return { channel: 'feishu', ...result };
    }

    const result = await sendMediaLark({ ...ctx, mediaUrl, mediaLocalRoots });
    return {
      channel: 'feishu',
  ...
  sendPayload: async ({ cfg, to, payload, mediaLocalRoots, accountId, replyToId, threadId }) => {
    ...
    if (feishuData?.card) {
      ...
      const cardResult = await sendCardLark({ ...ctx, card: feishuData.card });

      const warnings: string[] = [];
      for (const mediaUrl of mediaUrls) {
        const mediaResult = await sendMediaLark({ ...ctx, mediaUrl, mediaLocalRoots });
    ...
    for (const mediaUrl of mediaUrls) {
      lastResult = await sendMediaLark({ ...ctx, mediaUrl, mediaLocalRoots });
